package com.dogmaweb.springframework.web.bind.support;
...
/**
 * This class handles how session scoped model attributes are stored and retrieved from the HttpSession. This
 * implementation uses a timestamp to distinguish multiple command objects of the same type. This is needed for users
 * editing the same entity on multiple tabs of a browser. Source:
 * http://forum.springsource.org/showthread.php?95016-Using-Session-Model -Attributes-With-Multiple-Browser-Tabs-Patch
 * Note: Conversation management was on the agenda for 3.1 then got moved to 3.2 but doesn't seem like it will make it
 * there.
 * 
 * @author MJones
 * @version Sep 2, 2010 Note 2: This is a modification of the code in the URL because I don't want to use a tag lib.
 *          Therefore - my implementation creates a conversation map on the session - to allow each conversation to have
 *          its own store without adding the cid on the object name. In this way - the cid and the session attributes
 *          are kept separated.
...
public class ConversationalSessionAttributeStore implements SessionAttributeStore, InitializingBean {
    ...
    private static Logger logger = Logger.getLogger(ConversationalSessionAttributeStore.class);
    ...
    /**
     * Retrieve a specific conversation's map of objects from the session. Will create the conversation map if it does
     * not exist. The conversation map is stored inside a session map - which is a map of maps. If this does not exist
     * yet- it will be created too.
     * 
    ...
    /**
     * Store an object on the session. If the configured maximum number of live conversations to keep is reached - clear
     * out the oldest conversation. (If max number is configured as 0 - no removal will happen)
     * 
